...
- [x] Supabase Auth の設定
- [x] `/admin/login` ページの実装
- [x] 管理者ログイン機能（Server Action）
- [x] セッション管理・認証ミドルウェア
- [x] `/admin/settings` ページの実装
  - [x] クリニック設定の取得
  - [x] クリニック設定の更新（タイムゾーン、丸め単位、丸めモード）
- [x] 初期クリニックデータの投入スクリプト
...
- Phase 2: 認証・基本設定（2025-11-27）
  - Supabase Auth の設定とログイン機能（`app/actions/auth.ts`）
  - `/admin/login` ページの実装
  - 認証ミドルウェアの実装（`middleware.ts`）
  - `/admin/settings` ページの実装
  - クリニック設定の取得・更新機能（`app/actions/clinic.ts`）
  - 初期クリニックデータ投入スクリプト（`scripts/init-clinic.ts`）
  - 認証ユーティリティ（`lib/auth/utils.ts`）
  - `/admin/dashboard` ページの基本実装
